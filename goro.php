...
include("utilisateur.php"); ?>
    <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
      ...
      if(isset($_GET['id'])){
        $id_champ=$_GET['id'];
        $ville= $bdd ->prepare("SELECT * FROM sites_touristiques WHERE idville= :id");
        $ville->execute([
          'id' => $id_champ,
        ]);
        $titre= $bdd->prepare("SELECT nom FROM ville WHERE id= :id");
        $titre->execute([
          'id' => $id_champ,
        ]);
        $d= $titre->fetch();
        ...
        <table class="table table-striped table-sm">
         ...
         while($donnees= $ville->fetch()){
            ...
            <tr>
              ...
              <td><img class="magi" src="<?php echo $donnees['photo']; ?>" alt=""></td>
              <td><?php echo $donnees['content']; ?></td>

              <td><button style="background-color: yellow";>add</button></td>
